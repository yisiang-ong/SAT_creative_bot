...
#         'RobertaForEmotionclassification.pt', map_location=torch.device('cpu')), strict=False)


# #load emotion classifier (RoBERTa)
# with torch.no_grad():
#     emo_model = EmoClassificationModel(AutoModelWithLMHead.from_pretrained("roberta-base").base_model, len(labels))
#     emo_model.load_state_dict(torch.load('RoBERTa_emotion_best.pt', map_location=torch.device('cpu')), strict=False) #change path
...
#         'T5_small_empathy.pt', map_location=torch.device('cpu')), strict=False)  # change path
...
def get_sentence_score(sentence, dataframe):
    ...
    score = 1.5*empathy + 0.75*fluency + 2*novelty
    return score
